 ...
 * @protected
 */
CoreObject.prototype._setDataEntity = function(key, className, docs, isNullable) {
  if (docs == null || docs.length < 1) {
    ...
    for (var i = 0; i < docs.length; i++) {
      if (docs[i] === Object(docs[i])) {
        if (docs[i]._id == null) {
          throw new Error("Invalid identifier [Object in Array without _ID]");
        }
        docsEntity.push(Factory.getModel(className, docs[i]._id));
      } else if (docs[i] == null) {
        if (!isNullable) {
    ...
    this._setData(key, Factory.getModel(className, docs._id));
  } else {
    throw new Error("Invalid identifier");
  }
};
