...
CoreObject.prototype._getDataForSave = function(key) {
  return this._getDataForSaveByType(this._getData(key));
};

CoreObject.prototype._getDataForSaveByType = function(data) {
  if (data != null && (typeof data._getStructuredValueForSave) === 'function') {
    return data._getStructuredValueForSave();
  } else if (Array.isArray(data)) {
    var retData = [];
    for (var i = 0; i < data.length; i++) {
      retData.push(this._getDataForSaveByType(data[i]));
    }
    return retData;
  }
  return data;
